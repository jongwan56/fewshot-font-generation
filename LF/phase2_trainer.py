...
class LF2Trainer(BaseTrainer):
    ...
    def train(self, loader, val_loaders, max_step=100000):
        ...
        for batch in cyclize(loader):
            ...
            feat_styles, feat_comps = self.gen.encode_write_fact(
                ref_fids, ref_decs, ref_imgs, write_comb=True
            )
            self.add_fact_const_loss()

            feats_rc = (feat_styles * feat_comps).sum(1)
            gen_imgs = self.gen.read_decode(
                trg_fids, trg_decs, src_imgs, phase="fact", try_comb=True
            )
            ...
            self.add_ac_losses_and_update_stats(
                feats_rc, ref_decs, gen_imgs, trg_decs, stats
            )
    ...
    def add_fact_const_loss(self):
        loss = self.gen.get_fact_memory_var()
        self.g_losses['fact_const'] = loss * self.cfg["fact_const_w"]
        return loss
